...
### Figure 3- integrated (no temporal variability)
```{r}
...
air_df <- data.frame(type = levels(overview$landcover),
                       Means = tapply(X = overview$air_temperature, INDEX = overview$landcover,
                                        FUN = mean))
bite_df <- data.frame(type = levels(overview$landcover),
                       Means = tapply(X = overview$biting_rate, INDEX = overview$landcover,
                                        FUN = mean))
transmit_df <- data.frame(type = levels(overview$landcover),
                       Means = tapply(X = overview$transmission_prob, INDEX = overview$landcover,
                                        FUN = mean))
...
#### role of both spatial and temporal resolution
```{r}
all_pixels_integrate <- readRDS(here::here("risk_maps",
                               "data", 
                               "processed_data", 
...
#### Plot Jensen's inequality for spatial stuff through time: not included
```{r}
...
<!-- ```{r} -->
<!-- # spatial variability -->
<!-- Risks = c("air_temperature", "biting_rate", "transmission_prob") -->
<!-- new_names = c("Air temperature (C)", "Biting rate (1/day)", "Transmission probability (%)") -->
<!-- names(new_names) = Risks -->

<!-- All_longer <- All %>% -->
<!--   pivot_longer(cols = c("air_temperature", "biting_rate", "transmission_prob"), names_to = "type", values_to = "value") -->

<!-- All_longer$type <- as.factor(All_longer$type) -->

<!-- means_df <- data.frame(type = levels(All_longer$type), -->
<!--                        Means = tapply(X = All_longer$value, INDEX = All_longer$type, -->
<!--                                         FUN = mean)) -->

<!-- air_df <- means_df -->
<!-- air_df["biting_rate", "Means"] <- bite(air_df["air_temperature", "Means"]) -->
<!-- air_df["transmission_prob", "Means"] <- transmit(air_df["air_temperature", "Means"]) -->

<!-- All_longer %>% -->
<!--   ggplot() + -->
<!--   geom_histogram(aes(x = value), bins = 10, fill = "indianred1", alpha = .5) + -->
<!--   facet_wrap(vars(type), nrow = 3, scales = "free_x", labeller = labeller(type = new_names)) + -->
<!--   geom_vline(data = means_df, aes(xintercept = Means, colour = "Mean risk")) + -->
<!--   geom_vline(data = air_df, aes(xintercept = Means, colour = "Mean temperature risk"), linetype = "dashed") + -->
<!--   scale_color_manual(values = c("indianred1", "black")) + -->
<!--   theme_bw() + -->
<!--   ylab("Image count") + -->
<!--   xlab("Average image risk") + -->
<!--   theme(legend.position = c(.2, .16), legend.title = element_blank()) -->
<!-- ``` -->
...
<!-- all_pixels_integrate_longer <- all_pixels_integrate %>% -->
<!--   pivot_longer(cols = c("air_temperature", "biting_rate", "transmission_prob"), names_to = "type", values_to = "value") -->

<!-- all_pixels_integrate_longer$type <- as.factor(all_pixels_integrate_longer$type) -->

<!-- means_df <- data.frame(type = levels(all_pixels_integrate_longer$type), -->
<!--                        Means = tapply(X = all_pixels_integrate_longer$value, INDEX = all_pixels_integrate_longer$type, -->
<!--                                         FUN = mean)) -->
...
<!-- all_pixels_integrate_longer %>% -->
<!--   ggplot() + -->
<!--   geom_histogram(aes(x = value), bins = 10, fill = "steelblue1", alpha = .5) + -->
<!--   facet_wrap(vars(type), nrow = 3, scales = "free_x", labeller = labeller(type = new_names)) + -->
<!--   geom_vline(data = means_df, aes(xintercept = Means), colour = "steelblue1") + -->
<!--   geom_vline(data = air_df, aes(xintercept = Means), colour = "black", linetype = "dashed") + -->
<!--   # scale_color_manual(values = c("blue", "white")) + -->
<!--   theme_bw() + -->
<!--   ylab("Pixel count") + -->
<!--   xlab("Average pixel risk") -->
<!--   # theme(legend.position = c(.17, .14), legend.title = element_blank()) -->
<!-- ``` -->

### Figure Sx: Distribution of ECOSTRESS images included in the study by month and hour of day.
```{r}
