...
avg_bite <- all_pixels_integrate$air_temperature %>% mean() %>% bite
avg_transmit <- all_pixels_integrate$air_temperature %>% mean() %>% transmit
...
### Figure 4 - Distribution of crop types during the daytime
...
day_pixels$landcover <- factor(pixels$landcover, levels = c("Urban",  "Uncultivated", "Vegetable", "Fruit", "Orchard", "Field Crop", "Other"))
...
```{r}
f_air_temp <- aov(air_temperature ~ landcover, data = filter(day_pixels, landcover %in% ag_types))
f_trans_prob <- aov(transmission_prob ~ landcover, data = filter(day_pixels, landcover %in% ag_types))
