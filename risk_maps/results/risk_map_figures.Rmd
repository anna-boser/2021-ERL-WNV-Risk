...
ggplot(overview, aes(x = hour(dt) + minute(dt)/60, y = air_temperature, col = landcover, shape = landcover)) + 
      ...
      scale_color_manual(values = c(Urban = "#D55E00", All = "#E69F00", Agriculture = "#009E73")) + 
...
# These are not super helpful so I mught just not include them...

# ag_specs <- filter(landcover_avg, landcover %in% c("Fodder", "Fruit", "Fruit Tree", "Nuts", "Vegetable", "Uncultivated"))

# #ag plots
# ggplot(ag_specs, aes(x = hour(dt) + minute(dt)/60, y = air_temperature, col = landcover)) + 
#       geom_point() + 
#       # geom_smooth(method = "lm", 
#       #         formula = y ~ 
#       #           I(sin((2*pi*(x))/24)) + I(cos((2*pi*(x))/24)) + I(sin((2*2*pi*(x))/24)) + I(cos((2*2*pi*(x))/24)), 
...
#       guides(alpha = FALSE, col = guide_legend(title="Landcover type"))
# 
# ggplot(ag_specs, aes(x = hour(dt) + minute(dt)/60, y = biting_rate, col = landcover)) + 
#       geom_point() + 
#       # geom_smooth(method = "lm", 
#       #         formula = y ~ 
#       #           I(sin((2*pi*(x))/24)) + I(cos((2*pi*(x))/24)) + I(sin((2*2*pi*(x))/24)) + I(cos((2*2*pi*(x))/24)), 
...
#       guides(alpha = FALSE, col = guide_legend(title="Landcover type"))
# 
# ggplot(ag_specs, aes(x = hour(dt) + minute(dt)/60, y = transmission_prob, col = landcover)) + 
#       geom_point() + 
#       # geom_smooth(method = "lm", 
#       #         formula = y ~ 
#       #           I(sin((2*pi*(x))/24)) + I(cos((2*pi*(x))/24)) + I(sin((2*2*pi*(x))/24)) + I(cos((2*2*pi*(x))/24)), 
...
plot_densities <- function(type, formaltype, xlim, ylim){
  print(filter(all_pixels, landcover %in% "Urban") %>% 
