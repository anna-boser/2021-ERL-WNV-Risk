...
ggplot(Comp_temp, aes(x = lm3$fitted.values, y = lm3V$fitted.values)) + geom_point(aes(color = V), alpha = .2) + 
  ...
  ggtitle("Comparative modeled air temperature from a cubic land surface temperature model") + 
...
ggplot(Comp_temp, aes(x =  V, y =Air_Temp)) + 
  ...
  labs(title = "Fractional vegetation by air temperature") + 
...
ggplot(Comp_temp, aes(x = ECOSTRESS, y =Air_Temp)) + 
  ...
  stat_function(fun = function(x){summary(lm3)$coefficients[1,1] + summary(lm3)$coefficients[2,1]*x + summary(lm3)$coefficients[3,1]*x^2 + summary(lm3)$coefficients[4,1]*x^3 }, aes(colour = "Cubic regression"), size = 1, show.legend = TRUE) + 
...
```{r}
ggplot(Comp_temp, aes(x = ECOSTRESS, y =Air_Temp)) + 
  geom_point(aes(alpha = .2, color = ifelse(!is.na(Location), "CIMIS", "NOAA")), size = .8) +
  # labs(title = "ECOSTRESS LST vs. CIMIS air temperature") + 
  ylab(TeX("Air Temperature ($^o$C)")) + 
  xlab(TeX("Land Surface Temperature ($^o$C)")) + 
  ylim(min = 5, max = 45) + 
